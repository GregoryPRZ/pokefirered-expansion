...
static void SetSelectedMenu(void) {
  if (FlagGet(FLAG_DEXNAV_GET) == TRUE) {
...
void HeatStartMenu_Init(void) {
  ...
  if (GetSafariZoneFlag() == FALSE) { 
    if (FlagGet(FLAG_DEXNAV_GET) == FALSE && menuSelected == 0) {
      menuSelected = 255;
    }
...
static const u8 gText_PokedexHeat[] = _("  Pokédex");
...
static void HeatStartMenu_UpdateMenuName(void) {
  ...
  switch(menuSelected) {
    case MENU_POKETCH:
      ...
      break;
    case MENU_POKEDEX:
      AddTextPrinterParameterized(sHeatStartMenu->sMenuNameWindowId, 1, gText_PokedexHeat, 1, 0, 0xFF, NULL);
      break;
    case MENU_PARTY:
...
static u8 SaveConfirmOverwriteDefaultNoCallback(void)
{
    DisplayYesNoMenuWithDefault(); // Show Yes/No menu (No selected as default)
...
static u8 SaveSavingMessageCallback(void) {
  ShowSaveMessage(gText_SavingDontTurnOffPower, SaveDoSaveCallback);
  return SAVE_IN_PROGRESS;
}
...
static void ShowSaveInfoWindow(void) {
    ...
    // Print region name
    yOffset = 1;
    SaveStatToString(SAVE_STAT_LOCATION, gStringVar4, TEXT_COLOR_GREEN);
    AddTextPrinterParameterized(sSaveInfoWindowId, FONT_NORMAL, gStringVar4, 0, yOffset, TEXT_SKIP_DRAW, NULL);

    // Print player name
    yOffset += 16;
    AddTextPrinterParameterized(sSaveInfoWindowId, FONT_NORMAL, gSaveStatName_Player, 0, yOffset, TEXT_SKIP_DRAW, NULL);
    SaveStatToString(SAVE_STAT_NAME, gStringVar4, color);
    xOffset = GetStringRightAlignXOffset(FONT_NORMAL, gStringVar4, 0x70);
    PrintPlayerNameOnWindow(sSaveInfoWindowId, gStringVar4, xOffset, yOffset);

    // Print badge count
    yOffset += 16;
    AddTextPrinterParameterized(sSaveInfoWindowId, FONT_NORMAL, gSaveStatName_Badges, 0, yOffset, TEXT_SKIP_DRAW, NULL);
    SaveStatToString(SAVE_STAT_BADGES, gStringVar4, color);
    xOffset = GetStringRightAlignXOffset(FONT_NORMAL, gStringVar4, 0x70);
    ...
    if (FlagGet(FLAG_SYS_POKEDEX_GET) == TRUE)
    {
        // Print pokedex count
        yOffset += 16;
        AddTextPrinterParameterized(sSaveInfoWindowId, FONT_NORMAL, gSaveStatName_Pokedex, 0, yOffset, TEXT_SKIP_DRAW, NULL);
        SaveStatToString(SAVE_STAT_POKEDEX, gStringVar4, color);
        xOffset = GetStringRightAlignXOffset(FONT_NORMAL, gStringVar4, 0x70);
        AddTextPrinterParameterized(sSaveInfoWindowId, FONT_NORMAL, gStringVar4, xOffset, yOffset, TEXT_SKIP_DRAW, NULL);
    }
    ...
    // Print play time
    yOffset += 16;
    AddTextPrinterParameterized(sSaveInfoWindowId, FONT_NORMAL, gSaveStatName_Time, 0, yOffset, TEXT_SKIP_DRAW, NULL);
    SaveStatToString(SAVE_STAT_TIME, gStringVar4, color);
    xOffset = GetStringRightAlignXOffset(FONT_NORMAL, gStringVar4, 0x70);
    AddTextPrinterParameterized(sSaveInfoWindowId, FONT_NORMAL, gStringVar4, xOffset, yOffset, TEXT_SKIP_DRAW, NULL);
...
static void CB2_OpenDexNav(void) {
  DexNav_OpenGui(CB2_ReturnToFieldWithOpenMenu);
}
...
static void HeatStartMenu_OpenMenu(void) {
  switch (menuSelected) {
    case MENU_POKETCH:
      DoCleanUpAndChangeCallback(CB2_OpenDexNav);
      break;
    case MENU_POKEDEX:
